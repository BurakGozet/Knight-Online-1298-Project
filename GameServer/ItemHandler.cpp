...
void CUser::ItemMove(Packet & pkt)
{
	...
	case ITEM_COSP_TO_INVEN:
		if (bSrcPos >= COSP_MAX+MBAG_COUNT || bDstPos >= HAVE_MAX
			// Make sure that the item actually exists there.
...
void CUser::ItemMove(Packet & pkt)
{
	...
	case ITEM_COSP_TO_INVEN:
		...
		pSrcItem = &m_sItemArray[INVENTORY_COSP + bSrcPos];
		pDstItem = &m_sItemArray[SLOT_MAX + bDstPos];
		if (pDstItem->nNum == 508065593 && pDstItem->nNum == 508066593)
		{
			pDstItem = &m_sItemArray[0];
	...
	case ITEM_SLOT_INVEN:
		...
		break;
	case ITEM_COSP_TO_INVEN:
		if (pTable->m_bSlot == 107)
			UserLookChange(bSrcPos, pSrcItem->nNum = 508065593, 0);
		else if (pTable->m_bSlot == 105)
			UserLookChange(bSrcPos, pSrcItem->nNum = 508066593, 0);
		else
			UserLookChange(bSrcPos, 0, 0);
		break;
...
bool CUser::RunExchange(int nExchangeID, uint16 count)
{
	...
	if (pExchange != nullptr)
	{
		...
		temp_sOriginItemCount5 = pExchange->nOriginItemNum[5] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[5] : temp_sCount);
		temp_sOriginItemCount6 = pExchange->nOriginItemNum[6] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[6] : temp_sCount);
		temp_sOriginItemCount7 = pExchange->nOriginItemNum[7] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[7] : temp_sCount);
		temp_sOriginItemCount8 = pExchange->nOriginItemNum[8] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[8] : temp_sCount);
		temp_sOriginItemCount9 = pExchange->nOriginItemNum[9] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[9] : temp_sCount);
		temp_sOriginItemCount10 = pExchange->nOriginItemNum[10] == 0 ? 0 : (count == 0 ? pExchange->sOriginItemCount[10] : temp_sCount);
	}
