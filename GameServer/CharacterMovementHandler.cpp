...
void CUser::GetUserInfo(Packet & pkt)
{
	...
	//pkt << GetZoneID() << uint8(-1) << uint8(-1) << uint16(0) << uint16(0) << uint16(0) << uint16(0) << uint8(0) << uint16(0) << uint16(0) << uint8(0) << uint8(1);
	pkt << GetZoneID() << uint8(-1) << uint8(-1) << uint16(0) << uint16(0) << uint16(0) /*yeni eklemeler*/ << uint8(1) << uint8(isGenieActive());
	//pkt << GetZoneID() << uint8(1) << uint8(1) <<  uint8(1) << uint8(1)<< uint8(1) << uint8(1)<< uint8(1) << uint8(1) /*yeni eklemeler*/ << uint8(1) << uint8(isGenieActive());
}
...
void CUser::ZoneChange(uint16 sNewZone, float x, float z)
{
	...
	else if (sNewZone == ZONE_DARK_LAND || sNewZone == ZONE_KROWAZ_DOMINION || sNewZone == ZONE_RONARK_LAND_BASE || sNewZone == ZONE_ARDREAM || sNewZone == ZONE_PVP_EVENT || sNewZone == ZONE_JURAD_MOUNTAIN || sNewZone == ZONE_LOST_TEMPLE)
