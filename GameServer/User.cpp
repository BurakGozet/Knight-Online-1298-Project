...
void CUser::SendLoyaltyChange(int32 nChangeAmount /*= 0*/, bool bIsKillReward /* false */, bool bIsBonusReward /* false */, bool bIsAddLoyaltyMonthly /* true */)
{
	...
	{
		if (m_bPremiumType > 0 && (isInPKZone() || GetMap()->isWarZone()))
			GoldGain(PVP_BONUS_GOLD+PVP_BONUS_GOLD/2);
		else if (m_bPremiumType == 0 && (isInPKZone() || GetMap()->isWarZone()))
								...
								if (pParty != nullptr)
								{
									for (int i = 0; i < 8; i++)
									{
										if (pParty->uid[i] >= 0)
										{
											...
											if (GetNation() == KARUS)
												pUserParty->QuestV2MonsterCountAdd(KARUS);
											else if (GetNation() == ELMORAD)
												pUserParty->QuestV2MonsterCountAdd(ELMORAD);
										}
									}
								}
							}
...
void CUser::HandleMiningAttempt(Packet & pkt)
{
	...
	if (resultCode == MiningResultSuccess)
	{
		...
		else if (rate >= 25 && rate <= 35 && random <= 5000)
		{
			GiveItem(g_pMain->m_sling);
			sEffect = 13081; // "Item" effect
		}
		else if (rate >= 35 && rate <= 45 && random <= 6000)
		{
		...
		else if (rate >= 45 && rate <= 55 && random <= 7000)
		{
			if (pTable->m_iNum == MATTOCK)
		...
		else if (rate >= 55 && rate <= 65 && random <= 8000)
		{
			if (pTable->m_iNum == MATTOCK)
		...
		else if (rate >= 65 && rate <= 75 && random <= 9000)
		{
			if (pTable->m_iNum == MATTOCK)
		...
		else if (rate >= 85 && rate <= 95 && random <= 10000)
		{
			if (pTable->m_iNum == MATTOCK)
